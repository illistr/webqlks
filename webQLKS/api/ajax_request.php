...
function checkout() {
	...
	foreach($_SESSION['cart'] as $item) {
		...
		$sql = "insert into Order_Details(order_id, room_id, price, num, total_money) values ('$orderId', '$product_id', '$price', '$num', '$totalMoney')";
		execute($sql);
	}
...
function addToCart() {
	...
	if(!$isFind) {
		$sql = "select Room.*, Category.name as category_name from Room left join Category on Room.category_id = Category.id where Room.id = $id";
