...
$productId = getGet('id');
$sql = "select Room.*, Category.name as category_name from Room left join Category on Room.category_id = Category.id where Room.id = $productId";
$product = executeResult($sql, true);

$category_id = $product['category_id'];
$sql = "select Room.*, Category.name as category_name from Room left join Category on Room.category_id = Category.id where Room.category_id = $category_id order by Room.updated_at desc limit 0,4";
	...
	<div class="row">
		...
		<div class="col-md-6">
			...
			<ul style="display: flex; list-style-type: none; margin: 0px; padding: 0px;">
				...
				<li style="margin-left: 20px; border-left: solid #dad7d7 1px; font-size: 13pt; padding-top: 3px; padding-left: 20px;">7,635 Từng Đặt</li>
				<li style="margin-left: 20px; border-left: solid #dad7d7 1px; font-size: 13pt; padding-top: 3px; padding-left: 20px;">Còn Phòng</li>
				</ul>
			...
			<button class="btn btn-success" style="margin-top: 20px; width: 100%; border-radius: 0px; font-size: 30px;" onclick="addCart(<?=$product['id']?>, $('[name=num]').val())">
				<i class="bi bi-plus-circle"></i> ĐẶT PHÒNG
			</button>
...
<div class="container" style="margin-top: 20px; margin-bottom: 20px;">
	<h1 style="text-align: center; margin-top: 20px; margin-bottom: 20px;">PHÒNG LIÊN QUAN</h1>
	<div class="row">
	<?php
		foreach($lastestItems as $item) {
			echo '<div class="col-md-3 col-6 product-item">
					...
					<p><button class="btn btn-success" onclick="addCart('.$item['id'].', 1)" style="width: 100%; border-radius: 0px;"><i class="bi bi-plus-circle"></i> Thêm giỏ hàng</button></p>
				</div>';
		}
	?>
	</div>
